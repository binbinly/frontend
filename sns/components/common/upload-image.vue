				...
				<view class="uni-uploader__files">
					<block v-for="(image,index) in imageList" :key="index">
						...
						<view class="uni-uploader__file position-relative">
							<image class="uni-uploader__img rounded" :src="image.thumb" :data-src="image.thumb" @click="_previewImage(index)" mode="aspectFill"></image>
							...
							<progress class="pt-1" v-if="image.progress > 0 && image.progress < 100" :percent="image.progress" activeColor="green" stroke-width="8" />
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			_previewImage(index) {
				let imgArr = this.imageList.map(item=>{
					return item.big
				})
				uni.previewImage({
