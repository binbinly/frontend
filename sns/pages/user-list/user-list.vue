<template>
	<view>
		...
		<view class="flex align-center" style="height: 100rpx;">
			...
			{{item.name}} <text v-if="item.num > 0" class="ml-2 font-sm">({{item.num|formatNum}})</text></view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取用户相关数据
			getCounts(){
				this.$H.get('/user/count',{},{
					token:true,
					notoast:true
				}).then(res=>{
					this.tabBars[0].num = res.friend_count
					this.tabBars[1].num = res.follow_count
					this.tabBars[2].num = res.fens_count
				})
			},
			// 获取数据
			getData(){
			...
			getList(){
				...
				this.$H.get('/user/'+this.tabBars[index].key+'?p='+page,{},{
					token:true,
					noCheck:true
				})
				.then(res=>{
