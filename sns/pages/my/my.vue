<template>
	<view>
		...
		<!-- 已登录 -->
		<view v-else class="flex align-center p-2" hover-class="bg-light" @click="openSpace(user.user_id)">
		...
		<view class="px-3 py-2" v-for="(item, index) in info.bg" :key="index">
			<image :src="item.image" mode="aspectFill" style="height: 170rpx;width: 100%;" class="rounded"></image>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			sign(){
				return this.userinfo ? this.user.userinfo.sign : '无'
			}
		},
		onShow() {
			if (this.loginStatus) {
		...
		methods: {
			...
			getCounts() {
				this.$H.get('/user/my', {}, {
					...
					if (res) {
						this.myData[0].num = res.info.t_post_count
						this.myData[1].num = res.info.post_count
						this.myData[2].num = res.info.comment_count
						this.myData[3].num = res.info.fens_count
						this.totalPost = res.info.t_count
					}
					this.info = res
				})
			},
