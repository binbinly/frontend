<template>
	<view class="page">
		...
		<free-nav-bar showBack :showRight="detail.friend" bgColor="bg-white">
			<view slot="right">
				<free-icon-button :icon="'\ue6fd'" v-if="detail.friend" @click="openAction"></free-icon-button>
			</view>
		</free-nav-bar>
		...
		<free-divider></free-divider>
		<view class="py-3 flex align-center justify-center bg-white" hover-class="bg-light" v-if="detail.friend" @click="doEvent">
		...
		<view v-if="user.id != detail.id" class="py-3 flex align-center justify-center bg-white" hover-class="bg-light"
		...
		<free-popup ref="action" bottom transformOrigin="center bottom" maskColor>
			<scroll-view scroll-y="true" style="height: 580rpx;" class="bg-white" :show-scrollbar="false">
				<free-list-item v-for="(item,index) in actions" :key="index" :title="item.title" :showRight="false" :border="false"
	...
	export default {
		...
		methods: {
			getData() {
				$H.get('/friend/read/' + this.detail.id).then(res => {
					...
					if (res.moments && res.moments[0]) {
						...
						if (res.moments[0].image.length >= 4) { //截取4个元素
							res.moments[0].image = res.moments[0].image.splice(0, 4)
						}
					}
					this.detail = res
				})
			...
			doEvent() {
				...
				uni.navigateTo({
					url: '../../chat/chat/chat?id=' + this.detail.id + '&type=user',
				});
