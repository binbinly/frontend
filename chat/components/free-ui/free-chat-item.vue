<template>
	<div @longpress="long">
		...
		:class="!isself ? 'justify-start' : 'justify-end'">
			...
			<view v-if="isself && item.sendStatus && item.sendStatus !== 'success'" class="flex flex-column p-2">
				<text class="font-sm" :class="item.sendStatus === 'fail' ? 'text-danger' : 'text-muted'">{{item.sendStatus === 'fail' ? 'X' : ''}}</text>
			</view>
			<view class="flex flex-column">
				...
				<div class="p-2 rounded" :class="labelClass" style="max-width:500rpx;" :style="labelStyle">
					...
					<progress v-if="isself && item.progress > 0 && item.progress < 100" :percent="item.progress" activeColor="green" stroke-width="8" />
