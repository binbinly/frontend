		...
		<view class="pl-2 flex-1 flex flex-column">
			...
			<view v-if="item.image.length" class="py-2 flex flex-wrap">
				<block v-for="(image,imageIndex) in item.image" :key="imageIndex">
					<!-- 单图 -->
					<free-image v-if="item.image.length === 1" :src="image" mode="aspectFill" imageClass="rounded bg-secondary" @click="preview(image)"></free-image>
					<!-- 多图 -->
					<image v-else :src="image" mode="aspectFill" style="height: 180rpx;width: 180rpx;" class="bg-secondary mr-1 mb-1 rounded"
			...
			<view class="bg-light mt-2" v-if="item.likes.length || item.comments.length">
				...
				<view v-if="item.likes.length" class="border-bottom flex align-start p-2">
					<text class="flex-shrink iconfont text-hover-primary" style="font-size:10rpx;">&#xe637;</text>
					<view class="flex flex-1 flex-wrap ml-1">
						<text v-for="(s,si) in item.likes" :key="si" class="font-sm text-hover-primary ml-1">{{s.name}}</text>
					</view>
				</view>
				<!-- 评论 -->
					...
					<view class="flex flex-column flex-1 ml-2">
						<view class="flex" v-for="(c,ci) in item.comments" :key="ci">
							...
							<view v-else class="flex align-center">
								<text class="text-hover-primary font-sm">{{c.user.name}} </text>
								<text class="text-muted font-sm">回复</text>
								<text class="text-hover-primary font-sm">{{c.reply.name}}：</text>
							</view>
							<text class="font text-dark flex-1" @click="$emit('reply',{
								item,
								index,
								reply:c.user
							})">{{c.content}}</text>
